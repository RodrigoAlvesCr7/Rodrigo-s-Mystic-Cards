...
class PlayerHand {
    ...
    createCards(cardsInfo) {
        ...
        for (let cardInfo of cardsInfo) {
            ...
            else if (cardInfo.type == 4) {
                cards.push(new MagicCard(cardInfo.id, cardInfo.name, cardInfo.rarity, cardInfo.type, cardInfo.state, cardInfo.position, GameInfo.images.cards[cardInfo.cardID - 1], cardInfo.attack));
            }
        }
        return cards;
    }
    ...
    async release() {
        ...
        if (this.dragAction) {
            ...
            GameInfo.opponentBoard.releaseOpp();
...
class PlayerBoard extends PlayerHand {
    ...
    getDragCardBoardPos(cardX, cardY) {
        ...
        for (let i = 0; i < 3; i++) {
            if (this.x + (boardPosWidth * i) <= mouseX &&
                this.x + (boardPosWidth * i + 100) >= mouseX &&
                this.y <= mouseY &&
                (this.y + boardPosHeight * i + 200) > mouseY) {
                return i + 1;
            }
