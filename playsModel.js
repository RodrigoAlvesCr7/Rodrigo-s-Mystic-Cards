...
async function checkEndGame(game) {
    let [ug] = await pool.query(`select * from user_game where ug_game_id = ? and ug_id = ?`, [game.id, game.player.id]);
    console.log(ug[0])
    return ug[0].ug_hp <= Play.minHp;
}
...
class Play {
    ...
    static async getUserBoardInfo(userId, gameId) {
        ...
        for (let i = 0; i < playerInfo.hand.length; i++) {
            ...
            else if (playerInfo.hand[i].type === 4) {
                [[typeDataDB]] = await pool.query(
                    `Select cep_attack as "attack" from card_magic where cep_crd_id = ?`,
                    [playerInfo.hand[i].cardID]);
                playerInfo.hand[i].attack = typeDataDB.attack;
            }
        }
