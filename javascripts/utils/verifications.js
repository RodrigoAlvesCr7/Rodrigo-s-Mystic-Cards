    ...
    try {
        ...
        if (result.err) throw result.err
            ...
            if (window.location.pathname != "/matches.html") 
                changePage("matches.html","You finished your game ! Going to matches page.",verbose);
        } else {
            window.game = result.game;
            if (result.game.state == "Waiting") {
              if (window.location.pathname != "/waiting.html")
                changePage("waiting.html","You have created a game, now wait for the other player",verbose);
