...
class Card {
    ...
    static async playCard(game, ugcID, boardPos) {
        try {
            ...
            // magic card
            else if (cardData.typeID === 4) {
                result = await Card.cardMagic(ugcID, boardPos, game);
            }
    ...
    static async cardMagic(ugcID, boardPos, game) {
        try {

            let [[nerfedCard]] = await pool.query(
                `Select ugb_ugc_id as "ugcID"
                from user_game_board
                where ugb_position = ? and ugb_ug_id = ?`,
                [boardPos, game.opponents[0].id]);
            ...
            let [[nerf]] = await pool.query(
                `Select cep_attack 
                from card_magic
                where cep_crd_id =  ?`,
                [cardData.cardID]);

            let [[nerfedCardData]] = await pool.query(
                `Select uca_ap as "ap" from user_game_card_attack where uca_ugc_id = ?`,
                [nerfedCard.ugcID]);

            nerfedCardData.ap -= nerf.cep_attack;
            if (nerfedCardData.ap < 0) { nerfedCardData.ap = 0; }
            await pool.query(`Update user_game_card_attack set uca_ap = ? where uca_ugc_id = ?`,
                [nerfedCardData.ap, nerfedCard.ugcID]);
